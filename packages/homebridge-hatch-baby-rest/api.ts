...
export class HatchBabyApi {
  ...
  async createAwsIotClient() {
    ...
    logInfo(
      `[API] Got IoT token, region: ${iotResponse.region}, endpoint: ${iotResponse.endpoint}`,
    )
  ...
  async getDevices() {
    ...
    devices.forEach((d) =>
      logInfo(`[API]   - ${d.name} (${d.product}) MAC: ${d.macAddress}`),
    )

    const createDevices = <T extends IotDevice<any>>(
      product: Product,
      Device: new (
        info: IotDeviceInfo,
